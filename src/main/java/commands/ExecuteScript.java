...
public class ExecuteScript extends AbstractCommand {
    ...
    @Override
    public ActionResult action(CmdArgs args) throws FileNotFoundException, ValidException, InvocationTargetException, IllegalAccessException, ExecuteException {
        try {
            File file = getCurrentFileOrThrowValidException(args.getArgs());
            ...
            for (String[] cmdArgs : list.orElseThrow(() -> new ExecuteException("Failed to read file"))) {
                ...
                if (!Objects.isNull(command)) {
                    if (cmdArgs.length == 1) {
                        scriptResult.add(command.action(new CmdArgs()).getMessage());
                    } else {
                        List<String> argsList = new ArrayList<>(Arrays.asList(cmdArgs));
                        argsList.remove(0);
                        scriptResult.add(command.action(new CmdArgs(argsList.toArray(new String[0]))).getMessage());
                    }
                }
            }
        ...
        return new ActionResult(true, scriptResult.stream().collect(Collectors.joining(System.lineSeparator())));
    }
